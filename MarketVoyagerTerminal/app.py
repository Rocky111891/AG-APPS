...
INDICES_DATA = {
    ...
    "Bitcoin": {"country": "Global", "region": "Crypto", "symbol": "BTC-USD", "flag": "₿"},
    "Gold": {"country": "Global", "region": "Commodities", "symbol": "GC=F", "flag": "🥇"},
    "Crude Oil": {"country": "Global", "region": "Commodities", "symbol": "CL=F", "flag": "🛢️"},
}
...
def get_data(symbol):
    ...
    try:
        ...
        # Use Ticker object for better isolation
        ticker = yf.Ticker(symbol)
        hist = ticker.history(start="2000-01-01") # Fetch from 2000 to cover all crisis periods
        
        if hist.empty:
            print(f"No data received for {symbol}")
            return None, pd.DataFrame()
            
        # Ensure columns are flat (Ticker.history usually returns flat columns for single symbol)
        if isinstance(hist.columns, pd.MultiIndex):
            hist.columns = hist.columns.get_level_values(0)
        
        # Ensure 'Close' exists
        if 'Close' not in hist.columns:
            print(f"ERROR: 'Close' column not found for {symbol}")
            return None, pd.DataFrame()
            
        # Get scalar price
        close_val = hist['Close'].iloc[-1]
        price = float(close_val.iloc[0]) if isinstance(close_val, pd.Series) else float(close_val)
...
def get_top_movers(index_name, period="1D %"):
    ...
    if not constituents: return [], [], []  # Return 3 empty lists
    performances = []
    for ticker in constituents:
        perf = get_stock_performance(ticker, period)
        if perf: performances.append(perf)
    if not performances: return [], [], []  # Return 3 empty lists
...
def get_scalar(val):
    """Helper to ensure we get a scalar float from a potential Series"""
...
def calc_sigma(hist, current_move):
    try:
        ...
        # Ensure we're working with a Series for pct_change
        close_col = hist['Close']
        if isinstance(close_col, pd.DataFrame):
            close_col = close_col.iloc[:, 0]
            
        daily_returns = close_col.pct_change().dropna() * 100
...
def calc_live_1d(hist):
    try:
        ...
        current = get_scalar(hist['Close'].iloc[-1])
        prev_close = get_scalar(hist['Close'].iloc[-2])
...
def calc_intraday(hist):
    try:
        ...
        current = get_scalar(hist['Close'].iloc[-1])
        open_price = get_scalar(hist['Open'].iloc[-1])
...
def calc(hist, days):
    try:
        if len(hist) < 2: return None
        cur = get_scalar(hist['Close'].iloc[-1])
        idx = -days-1 if len(hist) > days else 0
        old = get_scalar(hist['Close'].iloc[idx])
...
def crisis_perf(hist, start, end):
    try:
        ...
        if len(c) >= 2:
            end_val = get_scalar(c['Close'].iloc[-1])
            start_val = get_scalar(c['Close'].iloc[0])
...
def create_chart(hist, name, start_date=None, end_date=None, title_suffix=""):
    ...
    if idx.tz is not None:
        idx = idx.tz_localize(None)
        # Create a copy with naive index for plotting
        hist = hist.copy()
        hist.index = idx
...
def get_widget_data():
    """Fetch VIX, Bitcoin, and Gold data for widgets"""
    widgets = {}
    symbols = {
        "VIX": "^VIX",
        "Bitcoin": "BTC-USD",
    ...
    for name, symbol in symbols.items():
        try:
            ...
            if not hist.empty:
                ...
                # Get scalar values robustly
                current = get_scalar(hist['Close'].iloc[-1])
                prev = get_scalar(hist['Close'].iloc[-2]) if len(hist) >= 2 else current
            ...
            widgets[name] = {"price": 0, "change": 0}
    
    return widgets

def get_trending_tickers():
    """Get top trending tickers from a predefined watchlist"""
    watchlist = ["AAPL", "TSLA", "NVDA", "MSFT", "GOOGL", "AMZN", "META", "AMD", "NFLX", "COIN"]
    trending = []
    
    for ticker in watchlist:
        perf = get_stock_performance(ticker, "1D %")
        if perf:
            trending.append(perf)
    
    # Sort by absolute change and return top 5
...
# Add CSS for marquee animation and styling
app.index_string = '''
<!DOCTYPE html>
<html>
    <head>
    ...
    dmc.Container([
        ...
        html.Div([
            ...
            html.Div([
                ...
                "paddingRight": "10px", "minWidth": "300px", "maxWidth": "70%",
                "borderRight": "1px solid #373a40"
            }),
            
            # Middle Panel: Details (30% width)
            html.Div([
                ...
                "width": "30%", "height": "calc(100vh - 150px)", 
                "marginLeft": "10px", "marginRight": "10px", "overflow": "hidden"
            }),
            
            # Right Panel: Widgets (20% width)
            html.Div([
                dmc.Paper(id="widgets-panel", p="md", shadow="md", withBorder=True, 
                         style={
                             "height": "100%", "overflowY": "auto",
                             "background": "linear-gradient(135deg, rgba(51, 154, 240, 0.05) 0%, rgba(138, 43, 226, 0.05) 100%)",
                             "backdropFilter": "blur(10px)",
                             "border": "1px solid rgba(255, 255, 255, 0.1)"
                         })
            ], style={
                "width": "20%", "height": "calc(100vh - 150px)", 
        ...
        for name, info in INDICES_DATA.items():
            ...
            if price and not hist.empty:
                ...
                row = {
                    ...
                    "Price": f"${price:,.2f}",
                    "Intraday %": intraday if intraday is not None else 0,
                    "1D %": live_1d if live_1d is not None else 0,
                    "1D Sigma": sigma if sigma is not None else 0,
                    "5D %": calc(hist, 5) if calc(hist, 5) is not None else 0,
                    "1M %": calc(hist, 21) if calc(hist, 21) is not None else 0,
                    "YTD %": calc(hist, 252) if calc(hist, 252) is not None else 0,
                    "1Y %": calc(hist, 252) if calc(hist, 252) is not None else 0,
                    "3Y %": calc(hist, 756) if calc(hist, 756) is not None else 0,
                }
                
                # Add crisis period data, converting None to 0 and ensuring float
                for crisis in CRISIS_PERIODS.keys():
                    perf = crisis_perf(hist, CRISIS_PERIODS[crisis]["start"], CRISIS_PERIODS[crisis]["end"])
                    row[crisis] = float(perf) if perf is not None else 0.0
...
def update_detail(active_cell, data):
    try:
        ...
        ticker_tape_wrapper = html.Div([
            ticker_tape,
            html.Div(tape_items, style={
                "display": "inline-flex",
                "animation": "marquee 30s linear infinite",
                "whiteSpace": "nowrap",
                "width": "100%",
        ...
        news_component = dmc.Stack([
            dmc.Group([
                dmc.Group([
                    dmc.Text(f"🎙️ Live Commentary: {index_name}", fw=700, size="sm", c="dimmed"),
                    dmc.Badge("ON AIR", color="red", variant="filled", size="xs", className="blink-me")
                ], gap="xs"),
                dmc.Button("🔊 Listen", id="speak-btn", variant="subtle", size="xs", color="gray")
            ], justify="space-between", mb="xs"),
            
            dmc.Group([
                dmc.Stack([
                    dmc.Avatar(src=avatar_url, size="lg", radius="xl"),
                    dmc.Text("Analyst", size="xs", c="dimmed", ta="center")
                ], align="center", gap=5),
                dmc.Stack([
                    *[dmc.Paper([
                        dmc.Text(n["title"], size="sm", fw=500),
                        dmc.Text(n["time"], size="xs", c="dimmed", mt=2)
                    ], p="xs", radius="md", withBorder=True, style={
...
@callback(
    Output("widgets-panel", "children"),
    [Input("refresh", "n_clicks"),
     Input("interval-component", "n_intervals")],
    prevent_initial_call=False,
)
def update_widgets(refresh, n_intervals):
    """Update widgets panel with VIX, Bitcoin, Gold, and trending tickers.
    Optimized to fetch data concurrently for faster loading."""
    try:
        # Fetch widget data (VIX, Bitcoin, Gold)
        ...
        # Market indicators widgets
        for name, data in widgets.items():
            color = "green" if data['change'] >= 0 else "red"
            widget_cards.append(
                dmc.Card([
                    dmc.Stack([
                        dmc.Text(name, size="xs", c="dimmed", fw=600),
                        dmc.Text(f"${data['price']:,.2f}" if name != "VIX" else f"{data['price']:.2f}", 
                                size="lg", fw=700, c="white"),
                        dmc.Badge(f"{data['change']:+.2f}%", color=color, variant="filled", size="sm")
                    ], gap="xs", align="start")
                ], p="sm", withBorder=True, mb="sm", style={
        ...
        if trending:
            ...
            for item in trending[:5]:
                color = "#40c057" if item['change'] >= 0 else "#fa5252"
                trending_items.append(
                    dmc.Group([
                        dmc.Text(item['ticker'], size="xs", fw=700, style={"width": "50px"}),
                        dmc.Text(f"{item['change']:+.2f}%", size="xs", fw=600, c=color)
                    ], justify="space-between", mb="xs")
                )
            
            widget_cards.append(
                dmc.Card([
                    dmc.Text("🔥 Trending", size="sm", fw=700, mb="sm", c="dimmed"),
                    dmc.Stack(trending_items, gap="xs")
                ], p="sm", withBorder=True, style={
                    "backgroundColor": "#25262b",
                    "borderColor": "#373a40"
                })
            )
        ...
        return [
            dmc.Title("Market Widgets", order=4, mb="md", c="dimmed"),
            *widget_cards
        ]
        
    except Exception as e:
        print(f"Error in update_widgets: {e}")
        traceback.print_exc()
        return dmc.Alert("Widget data unavailable", color="red", title="Error")
